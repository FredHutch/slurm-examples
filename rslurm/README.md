...
The provided example (also from the `rslurm` documentation with some minor
changes) is an example of one way of running a parallel algorithm.  Note that
this does not need to be run within a Slurm job: it can be run on a login host.
